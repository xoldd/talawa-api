	...
	t.field({
		...
		resolve: async (_parent, args, ctx) => {
			...
			if (currentUser.role === "administrator") {
				if (
					parsedArgs.input.caption !== undefined &&
					currentUserId !== existingPost.posterId
				) {
					throw new TalawaGraphQLError({
						extensions: {
				...
				if (currentUserOrganizationMembership.role === "administrator") {
					if (currentUserId !== existingPost.posterId) {
			...
			// Updated post not being returned means that either it was already updated or its `id` column was changed by external entities before this update operation.
