			...
			return await ctx.drizzleClient.transaction(async (tx) => {
				const [createdPost] = await tx
					...
					.values({
						...
						pinnedAt:
							parsedArgs.input.isPinned === undefined ? undefined : new Date(),
				...
				if (parsedArgs.input.attachments !== undefined) {
					...
					// Inserted post attachments not being returned is an external defect unrelated to this code. It is very unlikely for this error to occur.
					if (createdPostAttachments === undefined) {
						ctx.log.error(
							"Postgres insert operation unexpectedly returned an empty array instead of throwing an error.",
						);
						throw new TalawaGraphQLError({
